...
{
	[BepInPlugin("com.baominix.durango.original.npcfriendlist", "NPC Friend List Plugin", "1.1.6")]
	[BepInDependency("com.baominix.durango.original.logcontrol", BepInDependency.DependencyFlags.SoftDependency)]
	[BepInDependency("com.baominix.durango.original.partysystem", BepInDependency.DependencyFlags.SoftDependency)]
	public class NPCFriendListPlugin : BaseUnityPlugin
	{
		public static NPCFriendListPlugin Instance;
		...
		private int _kAmbientLastLineIndex = -1;
		private int _charlieAmbientLastLineIndex = -1;
		private bool _kJoinDialogPending = false;
		private bool _charlieJoinDialogPending = false;
		...
		private const float NPC_FORMATION_SIDE_OFFSET = 210f;
		private const float NPC_SEPARATION_RADIUS = 320f;
		private const float NPC_SEPARATION_CORE = 180f;
		private const float NPC_SEPARATION_SPEED = 380f;
		...
		private const float JOIN_AMBIENT_MIN_DELAY = 1f;
		private const float JOIN_AMBIENT_MAX_DELAY = 5f;
		...
		private float _partyStatusRefreshTimer = 0f;
		...
		private void Awake()
		{
			Instance = this;
			Log = Logger;
			new Harmony("com.baominix.durango.original.npcfriendlist").PatchAll(Assembly.GetExecutingAssembly());
			Logger.LogInfo("NPCFriendListPlugin v1.1.6 loaded. NPC friends, NPC party follow and rescue are owned here.");
		}
			...
			try
			{
				...
				if (MockPartyState.KInvited && !MockPartyState.KJoined)
				{
					...
					if (MockPartyState.KInviteTimer >= 2.5f)
					{
						...
						_kJoinDialogPending = true;
						MockPartyState.KInvited = false;
						MockPartyState.KInviteTimer = 0f;
						TriggerOnParty(partySystem);
						UIManager.SystemMsg(NpcFriendLocalization.Get("joined_party", "K"), 3f);
				...
				if (MockPartyState.CharlieInvited && !MockPartyState.CharlieJoined)
				{
					...
					if (MockPartyState.CharlieInviteTimer >= 2.5f)
					{
						...
						_charlieJoinDialogPending = true;
						MockPartyState.CharlieInvited = false;
						MockPartyState.CharlieInviteTimer = 0f;
						TriggerOnParty(partySystem);
						UIManager.SystemMsg(NpcFriendLocalization.Get("joined_party", "Charlie"), 3f);
						SoundManager.PlayEvent("ui_party_join");
				...
				UpdateNpcPartyStatus(partySystem, Time.deltaTime);
			}
			catch (Exception ex)
			{
		...
		private void SpawnNPCK(PlayerBehavior localPlayer)
		{
			try
			{
				Vector3 spawnPos = GetNpcFormationPosition(localPlayer, NpcPartyMember.K);
				...
				Singleton<AssetBundleManager>.Instance().RequestAsset(
					...
					delegate(UnityEngine.Object asset)
					{
						if (asset != null && _kObject != null)
						{
							...
							kModel.SetActive(false);
							DisableNpcPrefabAutonomy(kModel);
		...
		{
			float initialDelay = _kJoinDialogPending
				? UnityEngine.Random.Range(JOIN_AMBIENT_MIN_DELAY, JOIN_AMBIENT_MAX_DELAY)
				: UnityEngine.Random.Range(CHARLIE_AMBIENT_MIN_DELAY, CHARLIE_AMBIENT_MAX_DELAY);
			yield return WaitKAmbientDelay(initialDelay);
			...
			while (_kObject != null && _kAnimal != null && MockPartyState.KJoined && !IsNpcOffline(NpcPartyMember.K))
			{
				...
				{
					HideNpcBubble(NpcPartyMember.K);
					yield return new WaitForSeconds(1f);
					continue;
				}

				string rawLine = K_INDOOR_AMBIENT_LINES[PickKAmbientLineIndex()];
				ShowNpcRescueBubble(
					NpcPartyMember.K,
					LocalizeLine(rawLine),
					GetSelectedKPortraitPreset(),
					CHARLIE_AMBIENT_BUBBLE_DURATION);
				_kJoinDialogPending = false;
				...
				yield return WaitKAmbientDelay(nextDelay);
			}

			_kAmbientDialogCoroutine = null;
		}

		private IEnumerator WaitKAmbientDelay(float seconds)
		{
			float waitUntil = Time.time + Mathf.Max(0.1f, seconds);
		...
		private int PickKAmbientLineIndex()
		{
			int length = K_INDOOR_AMBIENT_LINES.Length;
			if (length <= 1)
			{
				_kAmbientLastLineIndex = 0;
				return 0;
			}

			int index = UnityEngine.Random.Range(0, length);
			if (index == _kAmbientLastLineIndex)
			{
				index = (index + 1 + UnityEngine.Random.Range(0, length - 1)) % length;
			}
			_kAmbientLastLineIndex = index;
			return index;
		}
		...
		private Vector3 GetNpcFormationPosition(PlayerBehavior localPlayer, NpcPartyMember member)
		{
			Vector3 side = (member == NpcPartyMember.K)
				? -localPlayer.transform.right
				: localPlayer.transform.right;
			Vector3 position = localPlayer.transform.position
				- localPlayer.transform.forward * FOLLOW_DIST
				+ side * NPC_FORMATION_SIDE_OFFSET;
			position.y = localPlayer.transform.position.y;
			return position;
		}

		private bool TryGetOtherNpcPosition(NpcPartyMember member, out Vector3 otherPos)
		{
			NpcPartyMember otherMember = (member == NpcPartyMember.K) ? NpcPartyMember.Charlie : NpcPartyMember.K;
			if ((otherMember == NpcPartyMember.K && (!MockPartyState.KJoined || _kObject == null || IsNpcOffline(otherMember)))
				|| (otherMember == NpcPartyMember.Charlie && (!MockPartyState.CharlieJoined || _charlieObject == null || IsNpcOffline(otherMember))))
			{
				otherPos = Vector3.zero;
				return false;
			}

			if (otherMember == NpcPartyMember.K)
			{
				otherPos = (_kAnimal != null) ? _kAnimal.CurrentPosition : _kObject.transform.position;
			}
			else
			{
				otherPos = (_charlieAnimal != null) ? _charlieAnimal.CurrentPosition : _charlieObject.transform.position;
			}
			return true;
		}

		private Vector3 GetNpcSeparationVelocity(NpcPartyMember member, Vector3 npcPos, PlayerBehavior localPlayer)
		{
			Vector3 otherPos;
			if (!TryGetOtherNpcPosition(member, out otherPos)) return Vector3.zero;

			Vector3 away = npcPos - otherPos;
			away.y = 0f;
			float distance = away.magnitude;
			if (distance >= NPC_SEPARATION_RADIUS) return Vector3.zero;

			if (distance < 0.01f)
			{
				away = (member == NpcPartyMember.K)
					? -localPlayer.transform.right
					: localPlayer.transform.right;
				distance = 0f;
			}
			...
			// Keep a small steering force at the outer edge, then strengthen it
			// smoothly as the NPCs get closer. This behaves like local avoidance
			// without adding physics bodies that would fight AnimalBehavior.
			float closeness = 1f - Mathf.Clamp01(distance / NPC_SEPARATION_RADIUS);
			float speed = NPC_SEPARATION_SPEED * Mathf.Lerp(0.25f, 1f, closeness);
			return away.normalized * speed;
		}

		private Vector3 KeepNpcOutsideSeparationCore(
			NpcPartyMember member,
			Vector3 candidatePos,
			PlayerBehavior localPlayer)
		{
			Vector3 otherPos;
			if (!TryGetOtherNpcPosition(member, out otherPos)) return candidatePos;

			Vector3 away = candidatePos - otherPos;
			away.y = 0f;
			float distance = away.magnitude;
			if (distance >= NPC_SEPARATION_CORE) return candidatePos;
			...
			Vector3 separatedPos = otherPos + away.normalized * NPC_SEPARATION_CORE;
			separatedPos.y = candidatePos.y;
			try
			{
				Vector3 correction = separatedPos - candidatePos;
				CollisionParam param = Collisions.CreateCollisionParam(candidatePos, correction);
				return candidatePos + Collisions.ProcessSimpleSliding(param);
			}
			catch
			{
				return separatedPos;
		...
		private void UpdateKFollow(PlayerBehavior localPlayer, float dt)
		{
			...
			bool isChasing = distance > chaseThreshold;
			Vector3 separationVelocity = GetNpcSeparationVelocity(NpcPartyMember.K, kPos, localPlayer);
			if (!isChasing && separationVelocity.sqrMagnitude < 0.001f)
			{
				if (_kIsMoving)
				{
					SetKAnimation("idle");
			...
			{
				Vector3 teleportPos = GetNpcFormationPosition(localPlayer, NpcPartyMember.K);
			...
			Vector3 velocity = separationVelocity;
			if (isChasing)
			{
				Vector3 dir = pPos - kPos;
				dir.y = 0f;
			...
			velocity = Vector3.ClampMagnitude(velocity, RUN_SPEED);
			float speed = velocity.magnitude;
			if (speed < 0.01f) return;

			// Movement with collision sliding (like PetAI.ProcessCollisionWithSliding),
			// now with an additional NPC-to-NPC avoidance steering component.
			...
			try
			{
				...
				if (moveDelta != Vector3.zero)
				{
					...
					if (isChasing && speed > 0f && actualSpeed / speed < 0.7f)
					{
						_stuckTimer += dt;
						if (_stuckTimer > STUCK_TIMEOUT)
						{
							// Teleport if stuck too long
							Vector3 unstuckPos = GetNpcFormationPosition(localPlayer, NpcPartyMember.K);
			...
			nextPos = KeepNpcOutsideSeparationCore(NpcPartyMember.K, nextPos, localPlayer);
		...
		private void SpawnNPCCharlie(PlayerBehavior localPlayer)
		{
			try
			{
				...
				Vector3 spawnPos = GetNpcFormationPosition(localPlayer, NpcPartyMember.Charlie);
				spawnPos.y = localPlayer.transform.position.y;
				...
				Singleton<AssetBundleManager>.Instance().RequestAsset(
					...
					delegate(UnityEngine.Object asset)
					{
						if (asset != null && _charlieObject != null)
						{
							...
							DisableNpcPrefabAutonomy(model);
		...
		private static void DisableNpcPrefabAutonomy(GameObject model)
		{
			if (model == null) return;

			// Story NPC prefabs contain their own looping ClientActorChat. Preview
			// copies used by Friend/Party UI must never register that dialogue in the
			// world, and in-world party NPCs use this plugin's timed dialogue instead.
			ClientActorChat[] chats = model.GetComponentsInChildren<ClientActorChat>(true);
			foreach (ClientActorChat chat in chats)
			{
				...
				{
					// The prefab may not have reached Start yet.
				}
				chat.StopAllCoroutines();
				chat.enabled = false;
			}

			ClientAnimalActor[] actors = model.GetComponentsInChildren<ClientAnimalActor>(true);
			foreach (ClientAnimalActor actor in actors)
			{
				if (actor != null) actor.enabled = false;
			}

			ClientInteractionQuest[] interactions = model.GetComponentsInChildren<ClientInteractionQuest>(true);
			foreach (ClientInteractionQuest interaction in interactions)
			{
		...
		{
			StopCharlieAmbientDialog();
			_charlieAmbientLastLineIndex = -1;
		...
		{
			float initialDelay = _charlieJoinDialogPending
				? UnityEngine.Random.Range(JOIN_AMBIENT_MIN_DELAY, JOIN_AMBIENT_MAX_DELAY)
				: UnityEngine.Random.Range(CHARLIE_AMBIENT_MIN_DELAY, CHARLIE_AMBIENT_MAX_DELAY);
			yield return WaitCharlieAmbientDelay(initialDelay);
			...
			while (_charlieObject != null && _charlieAnimal != null && MockPartyState.CharlieJoined)
			{
				...
				{
					HideNpcBubble(NpcPartyMember.Charlie);
					yield return new WaitForSeconds(1f);
					continue;
				}

				int lineIndex = PickCharlieAmbientLineIndex();
				string rawLine = CHARLIE_AMBIENT_LINES[lineIndex];
				ShowNpcRescueBubble(
					NpcPartyMember.Charlie,
					LocalizeLine(rawLine),
					CHARLIE_PORTRAIT_PRESET,
					CHARLIE_AMBIENT_BUBBLE_DURATION);
				_charlieJoinDialogPending = false;

				yield return new WaitForSeconds(CHARLIE_AMBIENT_BUBBLE_DURATION);
				HideNpcBubble(NpcPartyMember.Charlie);

				float nextDelay = UnityEngine.Random.Range(CHARLIE_AMBIENT_MIN_DELAY, CHARLIE_AMBIENT_MAX_DELAY);
				yield return WaitCharlieAmbientDelay(nextDelay);
			}

			_charlieAmbientDialogCoroutine = null;
		}

		private IEnumerator WaitCharlieAmbientDelay(float seconds)
		{
			float waitUntil = Time.time + Mathf.Max(0.1f, seconds);
			while (Time.time < waitUntil && _charlieObject != null && MockPartyState.CharlieJoined && !IsNpcOffline(NpcPartyMember.Charlie))
			{
				yield return new WaitForSeconds(0.25f);
			}
		}

		private int PickCharlieAmbientLineIndex()
		{
			int length = CHARLIE_AMBIENT_LINES.Length;
			if (length <= 1)
			{
				_charlieAmbientLastLineIndex = 0;
				return 0;
			}

			int index = UnityEngine.Random.Range(0, length);
			if (index == _charlieAmbientLastLineIndex)
			{
				index = (index + 1 + UnityEngine.Random.Range(0, length - 1)) % length;
			}
			_charlieAmbientLastLineIndex = index;
			return index;
		}
		...
		private string LocalizeLine(string text)
		{
			...
			try
			{
				return NpcFriendLocalization.LocalizeOriginal(text);
			}
			catch
			{
				return text;
		...
		private void UpdateCharlieFollow(PlayerBehavior localPlayer, float dt)
		{
			...
			bool isChasing = distance > chaseThreshold;
			Vector3 separationVelocity = GetNpcSeparationVelocity(NpcPartyMember.Charlie, npcPos, localPlayer);
			if (!isChasing && separationVelocity.sqrMagnitude < 0.001f)
			{
				if (_charlieIsMoving)
				{
			...
			if (distance > TELEPORT_DIST)
			{
				Vector3 teleportPos = GetNpcFormationPosition(localPlayer, NpcPartyMember.Charlie);
			...
			velocity = Vector3.ClampMagnitude(velocity, RUN_SPEED);
			float requestedSpeed = velocity.magnitude;
			if (requestedSpeed < 0.01f) return;
			...
			try
			{
				...
				float actualSpeed = slidDelta.magnitude / dt;
				if (isChasing && actualSpeed / requestedSpeed < 0.7f)
				{
					_charlieStuckTimer += dt;
					if (_charlieStuckTimer > STUCK_TIMEOUT)
					{
						Vector3 unstuckPos = GetNpcFormationPosition(localPlayer, NpcPartyMember.Charlie);
			...
			nextPos = KeepNpcOutsideSeparationCore(NpcPartyMember.Charlie, nextPos, localPlayer);
		...
		private void UpdateNpcOfflineTransition(NpcPartyMember member, PartySystem partySystem, ref bool wasOffline)
		{
			...
			if (offlineUntil > 0f)
			{
				...
				if (wasOffline && IsNpcJoined(member))
				{
					...
					UIManager.SystemMsg(NpcFriendLocalization.Get("online_again", npcName), 3f);
		...
		private void SetNpcOfflineAfterRescue(NpcPartyMember member)
		{
			...
			PartySystem partySystem = GameSystem<PartySystem>.Instance();
			TriggerOnParty(partySystem);
			UIManager.SystemMsg(NpcFriendLocalization.Get("offline_minute", npcName), 3f);
		...
		private string PickLine(string[] lines)
		{
			...
			return NpcFriendLocalization.Get(
		...
		private IEnumerator CoNpcPartyRescue()
		{
			...
			{
				UIManager.SystemMsg(NpcFriendLocalization.Get("rescue_in_progress"), 3f);
				yield break;
			}
			...
			while (true)
			{
				...
				catch (Exception ex)
				{
					...
					UIManager.SystemMsg(NpcFriendLocalization.Get("rescue_failed"), 3f);
					break;
				}
		...
		private IEnumerator CoNpcPartyRescueImpl()
		{
			...
			if (player == null)
			{
				UIManager.SystemMsg(NpcFriendLocalization.Get("no_local_player"), 3f);
				yield break;
			}
			if (player.IsAlive)
			{
			...
			{
				UIManager.SystemMsg(NpcFriendLocalization.Get("no_rescuer"), 3f);
				yield break;
			}
			...
			{
				UIManager.SystemMsg(NpcFriendLocalization.Get("rescuer_not_ready", GetNpcDisplayName(member)), 3f);
				yield break;
			}
			...
			UIManager.SystemMsg(NpcFriendLocalization.Get("coming_to_rescue", npcName), 3f);
		...
		private void ReviveLocalPlayer(PlayerBehavior player, string rescuerName)
		{
			...
			UIManager.SystemMsg(NpcFriendLocalization.Get("rescued", rescuerName), 3f);
		...
		private void UpdateNpcPartyStatus(PartySystem partySystem, float dt)
		{
			if (partySystem == null || dt <= 0f) return;
			if (!MockPartyState.IsInParty || (!MockPartyState.KJoined && !MockPartyState.CharlieJoined))
			{
				_partyStatusRefreshTimer = 0f;
				return;
			}

			_partyStatusRefreshTimer -= dt;
			if (_partyStatusRefreshTimer > 0f) return;
			_partyStatusRefreshTimer = 1f;

			PlayerBehavior localPlayer = PlayerBehavior.LocalPlayer;
			if (MockPartyState.KJoined)
			{
				TriggerOnPartierStatus(partySystem, BuildNpcPartierStatus(NpcPartyMember.K, localPlayer));
			}
			if (MockPartyState.CharlieJoined)
			{
				TriggerOnPartierStatus(partySystem, BuildNpcPartierStatus(NpcPartyMember.Charlie, localPlayer));
			}
		}

		private static Messages.PartierStatus BuildNpcPartierStatus(NpcPartyMember member, PlayerBehavior localPlayer)
		{
			bool online = !IsNpcOffline(member);
			return new Messages.PartierStatus
			{
				EntityId = (member == NpcPartyMember.Charlie) ? "npc_charlie" : "npc_k",
				RegionId = (GameManager.Region != null) ? GameManager.Region.Id : "region_0",
				Tile = GetNpcCurrentTile(member, localPlayer),
				...
				ExpiresAt = online ? double.MaxValue : 0.0
			};
		}

		private static Point2 GetNpcCurrentTile(NpcPartyMember member, PlayerBehavior localPlayer)
		{
			try
			{
				Vector3 position;
				if (Instance != null && Instance.TryGetNpcWorldPosition(member, out position))
				{
					return new Point2(Durango.Terrain.Util.ClientPositionToTilePosition(position));
				}
			}
			catch (Exception ex)
			{
				if (Instance != null) Instance.Logger.LogWarning("GetNpcCurrentTile failed: " + ex.Message);
			}

			return localPlayer != null ? localPlayer.CurrentTile : new Point2(0, 0);
		}

		private bool TryGetNpcWorldPosition(NpcPartyMember member, out Vector3 position)
		{
			AnimalBehavior animal = GetNpcAnimal(member);
			if (animal != null)
			{
		...
		private static void TriggerOnPartierStatus(PartySystem partySystem, Messages.PartierStatus status)
		{
			if (partySystem == null) return;
			var method = typeof(PartySystem).GetMethod("OnPartierStatus", BindingFlags.NonPublic | BindingFlags.Instance);
			if (method != null)
			{
				method.Invoke(partySystem, new object[] { status, null });
				return;
			}
		...
		public static void TriggerOnParty(PartySystem partySystem)
		{
			...
			if (MockPartyState.IsInParty)
			{
				...
				if (MockPartyState.KInvited || MockPartyState.KJoined)
				{
					Messages.PartierStatus kStatus = BuildNpcPartierStatus(NpcPartyMember.K, localPlayer);
					members.Add(new Pair<Messages.PartierStatus, bool>(kStatus, MockPartyState.KJoined));
				}

				if (MockPartyState.CharlieInvited || MockPartyState.CharlieJoined)
				{
					Messages.PartierStatus charlieStatus = BuildNpcPartierStatus(NpcPartyMember.Charlie, localPlayer);
			...
			private static void Postfix()
			{
				try
				{
					...
					menuList.Add(new InteractionMenuData(Interaction.Resurrect)
					{
						...
						Name = NpcFriendLocalization.Get("rescue_menu"),
						Icon = "act_Resurrect"
					});
			...
			private static bool Prefix(PartySystem __instance, string entityId)
			{
				...
				try
				{
					...
					if (entityId == "npc_k")
					{
						...
						MockPartyState.KInviteTimer = 0f;
						TriggerOnParty(__instance);
						UIManager.SystemMsg(NpcFriendLocalization.Get("invite_sent", "K"), 3f);
					...
					else if (entityId == "npc_charlie")
					{
						...
						MockPartyState.CharlieInviteTimer = 0f;
						TriggerOnParty(__instance);
						UIManager.SystemMsg(NpcFriendLocalization.Get("invite_sent", "Charlie"), 3f);
			...
			private static bool Prefix(PartySystem __instance, string entityId)
			{
				...
				try
				{
					if (entityId == "npc_k")
					{
						...
						ResetKPrefabSelection();
						TriggerOnParty(__instance);
						UIManager.SystemMsg(NpcFriendLocalization.Get("kicked", "K"), 3f);
					...
					else if (entityId == "npc_charlie")
					{
						...
						Instance._charlieJoinDialogPending = false;
						TriggerOnParty(__instance);
						UIManager.SystemMsg(NpcFriendLocalization.Get("kicked", "Charlie"), 3f);
		...
		// ================================================================
		// HARMONY PATCHES — UI preview isolation
		// ================================================================

		// UIModelRender places preview models far outside the visible world. Keep
		// every child, including Durango's cloned PlaneShadows meshes, on the same
		// preview-only layer. The shadow remains visible in Party Preview but cannot
		// leak into the real scene through the clones' default world layer 0.
		[HarmonyPatch(typeof(UIModelRender), "SetModel")]
		private static class UIModelRenderSetModelLayerIsolationPatch
		{
			private static void Postfix(UIModelRender __instance, GameObject obj)
			{
				if (__instance == null || obj == null) return;
				NormalizeUiPreviewLayer(__instance, obj);
				if (Instance != null)
				{
					Instance.StartCoroutine(KeepUiPreviewLayerNormalized(__instance, obj));
				}
			}
		}

		// Durango also creates its own flattened shadow meshes through
		// PlaneShadows/MeshCloner. Those clones default to world layer 0 even when
		// their source model belongs to UIModelRender (layer 13). Correct newly made
		// clones immediately after costumes or equipment are attached.
		[HarmonyPatch(typeof(Durango.Render.PlaneShadows), "Add")]
		private static class PlaneShadowsAddUiPreviewIsolationPatch
		{
			private static void Postfix(Durango.Render.PlaneShadows __instance)
			{
				if (__instance == null) return;
				UIModelRender render = __instance.GetComponentInParent<UIModelRender>();
				if (render == null) return;
				NormalizeUiPreviewLayer(render, __instance.gameObject);
			}
		}

		private static IEnumerator KeepUiPreviewLayerNormalized(UIModelRender render, GameObject model)
		{
			// PlayerManager.SetDisplay can attach costumes, weapons and accessories
			// after SetModel has returned. Re-apply the preview layer while active.
			while (render != null && model != null && render.gameObject.activeInHierarchy)
			{
				NormalizeUiPreviewLayer(render, model);
				yield return new WaitForSeconds(0.2f);
			}
		}

		private static void NormalizeUiPreviewLayer(UIModelRender render, GameObject model)
		{
			if (render == null || model == null) return;
			try
			{
				NGUITools.SetLayer(model, render.gameObject.layer);
			}
			catch (Exception ex)
			{
				if (Instance != null)
				{
					Instance.Logger.LogWarning("UI preview layer isolation failed: " + ex.Message);
				}
			}
		}

		// ================================================================
		// HARMONY PATCHES — PlayerInfoManager (provide NPC party member info)
		// ================================================================
