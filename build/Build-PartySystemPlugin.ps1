...
$SourceDir = Join-Path $pluginsDir "PartySystemPlugin"
...
$outDll = Join-Path $pluginDir "PartySystemPlugin.dll"
$csc = "C:\Windows\Microsoft.NET\Framework\v3.5\csc.exe"
$sources = @(Get-ChildItem -LiteralPath $SourceDir -Filter "*.cs" -Recurse -File | Select-Object -ExpandProperty FullName)
...
$refs = @(
    ...
    (Join-Path $refsDir "UnityEngine.AnimationModule.dll"),
	(Join-Path $refsDir "UnityEngine.PhysicsModule.dll"),
    (Join-Path $refsDir "MsgPack.dll")
)
